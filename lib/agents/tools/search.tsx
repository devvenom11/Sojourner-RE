...
import { searchSchema } from '@/lib/schema/search'; // Updated schema for validating search parameters
...
export const searchTool = ({ uiStream }: ToolProps) =>
  tool({
    ...
    execute: async ({
      ...
      // Since searchSchema now ensures max_results is a number, this check can be simplified
      const maxResults = max_results; // Already a number due to schema coercion
...
// Helper functions (tavilySearch, exaSearch, searxngSearch) remain unchanged
// Ensure they correctly handle the now-numbered maxResults parameter
